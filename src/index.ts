...
import { storeTasksToLocal, storeProjectsToLocal, retrieveLocalTasks, retrieveLocalProjects } from "./services/LocalStorage";
import { Task } from "./types/TaskTypes"
import { Header } from "./components/Header"
import { Sidebar } from "./components/Sidebar";
import { TaskList } from "./components/Tasklist";
import { TaskPane, toggleTaskPane } from "./components/TaskPane"
import { Modal, toggleModal } from "./components/Modal";
import { statuses, priorities, Status, Priority } from "./types/AppTypes";
import { clearContent, createElement, createInputField, createSelectField, createTextareaField, safeQuerySelector } from "./utils/domUtils"
...
const listProjects = () => {
  projectService.addProject({ id: 1, name: "🏠 Inbox" });
...
const initializeState = () => {
  const storedProjects = retrieveLocalProjects();
  const storedTasks = retrieveLocalTasks();

  if (storedProjects.length === 0) {
    projectService.addProject({ id: 1, name: "🏠 Inbox" });
  } else {
    storedProjects.forEach((project) => projectService.addProject(project));
  }

  storedTasks.forEach((task) => taskService.addTask(task))
}

function renderApp() {
  const { isTaskPaneOpen, isModalOpen } = useAppState.getState();
  ...
  const sidebar = Sidebar(
    ...
    (projectId) => {
      useAppState.getState().setActiveProjectId(projectId);
      renderApp();
    },
    (filter) => {
  ...
  const taskList = TaskList(taskService.getAllTasks(), renderApp);

  moduleContentDiv.append(header, sidebar, taskList);

  if (isTaskPaneOpen) {
    const taskPane = TaskPane(renderApp);
    moduleContentDiv.appendChild(taskPane);
  }

  modalContentDiv.style.display = isModalOpen ? "block" : "none";
}
...
function initializeGlobalListeners() {
  document.addEventListener("mousedown", (event: MouseEvent) => {
    ...
    const { isModalOpen, isTaskPaneOpen } = useAppState.getState();
    if (isModalOpen || !isTaskPaneOpen) return;

    if (!taskPane.contains(target)) {
      useAppState.setState({ isTaskPaneOpen: false });
    }
  });
}
...
const initializeApp = () => {
  ...
  // Create toggle function in module
  appState.isTaskPaneOpen
    ? moduleContentDiv.querySelector(".task-pane")?.classList.remove("display-none")
    : moduleContentDiv.querySelector(".task-pane")?.classList.add("display-none");
}
